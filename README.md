...
await search_index_manager.build_embeddings_file(
    ...
    sentence_per_embedding=4
)
```
Please also note the parameter `sentence_per_embedding`, which is the number of sentences, used to build the embedding. The bigger is this number - the bigger will be the context found by similarity search.
